...
const ProjectCard =({ item, setSelectedId }: { item: Item; setSelectedId: (id: string) => void })=>{
...
export const Projects =()=>{
    ...
    const draw = useTransform(scrollYProgress, [0.32, 0], [0, 2000])
    ...
    <>
      <section id="projects" className={styles.projects}>
        <div className={styles.container}>
          <motion.svg ref={ref} className={styles.svg} viewBox="0 0 569 215">
            <motion.path 
          ...
          <AnimatePresence>
            {selectedId && (
              <motion.div className={styles.detailContainer} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=> setSelectedId(null)}>
                <motion.div layoutId={selectedId} className={styles.detail} onClick={(e) => e.stopPropagation()} >
                  {projects
