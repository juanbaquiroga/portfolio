...
export const About_me = () => {
    ...
    const [textInView1, setTextInView1] = useState(false);
    const [textInView2, setTextInView2] = useState(false);
    ...
    useEffect(() => {
        ...
        const observerText1 = new IntersectionObserver(
            ([entry]) => {
                setTextInView1(entry.isIntersecting);
            },
            {
                rootMargin: "100000px 0px -100px 0px",
            }
        ...
        const observerText2 = new IntersectionObserver(
            ([entry]) => {
                setTextInView2(entry.isIntersecting);
            },
            {
                rootMargin: "100000px 0px -100px 0px",
            }
        ...
        if (refImg.current) observerImg.observe(refImg.current);
        if (refText1.current) observerText1.observe(refText1.current);
        if (refText2.current) observerText2.observe(refText2.current);
        if (refButton.current) observerButton.observe(refButton.current);

        return () => {
            if (refImg.current) observerImg.unobserve(refImg.current);
            if (refText1.current) observerText1.unobserve(refText1.current);
            if (refText2.current) observerText2.unobserve(refText2.current);
            if (refButton.current) observerButton.unobserve(refButton.current);
        };
    }, []);
