...
import { getTechnologies } from "@/lib/firebase";
...
export const Technologies = ({ isMobile }: { isMobile: boolean }) => {
  ...
  useEffect(()=>{
        const fetchData= async ()=>{
            try{
                setLoading(true);
                const projectsData = await getTechnologies();
                setTechnologies(projectsData);
                setError(null);
            }catch(err){
    ...
    if (subtitleRef.current) {
      if (subtitleInView) {
        gsap.fromTo(
          subtitleRef.current,
          { scale: 0, opacity: 0},
          { scale: 1, opacity: 1, duration: 0.6, ease: "elastic.out(1.05, 0.8)" }
        );
      } else {
      ...
      <section id="technologies" className={styles.technologies}>
        <div className={styles.container}>
          ...
          {isMobile && technologies ? (
            ...
            <Slider items={technologies} direction="left" speed="normal" />
          )}
        </div>
      </section>
    </>
